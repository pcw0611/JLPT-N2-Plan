# -*- coding: utf-8 -*-
"""Record Anki study stats and study session for 2026-09-22."""
...
TEMP_DIR = Path(r'C:\Users\pcw06\AppData\Local\Temp\anki_inspect_0922')
...
s_dt = datetime(2026, 9, 22, 4, 0, 0, tzinfo=KST)
e_dt = datetime(2026, 9, 23, 4, 0, 0, tzinfo=KST)
...
SUMMARY_PAYLOAD = {
    ...
    'sourceNote': (
        ...
        f'고유 카드 {distinct_cards}장 회독. '
        f'N2 집중 회독 1,953회(313장, 282.9분 / 4시간 43분, 1,169장 전량 1차 완독 회독 유지). '
        f'N3 332회(89장, 50.2분), N4 52회(19장, 5.4분), N5 33회(19장, 2.0분), N2 문법 예문 24회(24장, 4.5분), '
        f'공통 의성어/의태어 9회(2장, 0.8분), 공통 의상 7회(4장, 0.7분).'
    ),
    'inventory': {
        'n3_unseen': 0,
        'n2_unseen': 0,
...
SESSION_SUMMARY = (
    ...
    f"Anki 단어·문법 세션 {cnt:,}회 실측 (고유 카드 {distinct_cards}장): "
    f"1) JLPT 한끝 Voca 4-N2 고밀도 회독 1,953회(313장, 282.9분 / 4시간 43분, 1,169장 1차 완독 전량 회독 유지). "
    f"2) N3 332회(89장, 50.2분), N4 52회(19장, 5.4분), N5 33회(19장, 2.0분), N2 문법 001-150 예문 복습 24회(24장, 4.5분), "
    f"공통 의성어·의태어 9회(2장, 0.8분), 공통 의상 7회(4장, 0.7분) 복습 유지. "
