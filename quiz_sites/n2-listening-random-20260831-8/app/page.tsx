...
const TEST_ID = 'n2-listening-random-20260831-8';
...
export default function Home() {
  ...
  if (phase === 'intro') {
    return (
      <main className="shell intro-shell">
        <div className="exam-mark">JLPT N2 · LISTENING CHECK</div>
        <section className="intro-card">
          <p className="kicker">2026年8月31日</p>
          <h1>N2 청해 랜덤<br /><span>8問・連続試験</span></h1>
          <p className="intro-copy">前回と同じ問題を避け、新しいN2レベルの場面で聞き取りと即時判断を確認します。</p>
          <div className="intro-grid">
            <div><b>8</b><span>問題</span></div>
            <div><b>1</b><span>連続タイマー</span></div>
            <div><b>1回</b><span>聴解再生</span></div>
          </div>
          <ol className="sequence">
            <li><span>01</span>課題理解</li>
            <li><span>02</span>ポイント理解</li>
            <li><span>03</span>概要理解</li>
            <li><span>04</span>即時応答</li>
          </ol>
  ...
  if (phase === 'quiz') {
    const question = session[current];
    const sectionName = current < 3 ? '課題理解' : current < 5 ? 'ポイント理解' : current < 6 ? '概要理解' : '即時応答';
  ...
  return (
    <main className="shell result-shell">
      <header className="result-hero">
        <div><p className="kicker">採点完了 · 자체 제작 N2 청해 랜덤</p><h1>{score}<span> / {TOTAL}</span></h1><strong>{accuracy}%</strong></div>
      ...
      <footer>이 결과는 강의 직후 자체 제작 청해 확인이며 공식 JLPT 점수로 환산하지 않습니다.</footer>
    </main>
  );
}
