# -*- coding: utf-8 -*-
"""Record Anki study stats and study session for 2026-09-16 (236.72 min / 2,777 reviews)."""

import sqlite3
import json
import shutil
import sys
from pathlib import Path

if hasattr(sys.stdout, 'reconfigure'):
    sys.stdout.reconfigure(encoding='utf-8')

ROOT = Path(r'c:\Users\pcw06\Documents\Codex\JLPT-N2-Plan')
DB = ROOT / 'database' / 'jlpt_learning.db'
DATE = '2026-09-16'

# Inspect remaining cards from Anki collection
src = Path(r'C:\Users\pcw06\AppData\Roaming\Anki2\사용자 1')
dst = Path(r'C:\Users\pcw06\AppData\Local\Temp\anki_inspect_0916')
dst.mkdir(exist_ok=True)
shutil.copy2(src / 'collection.anki2', dst / 'collection.anki2')
if (src / 'collection.anki2-wal').exists():
    shutil.copy2(src / 'collection.anki2-wal', dst / 'collection.anki2-wal')

con_anki = sqlite3.connect(dst / 'collection.anki2')
cur_anki = con_anki.cursor()

cur_anki.execute('SELECT queue, count(*) FROM cards GROUP BY queue')
queue_counts = dict(cur_anki.fetchall())
print('Queue counts:', queue_counts)

new_rem = queue_counts.get(0, 0)
learn_rem = queue_counts.get(1, 0) + queue_counts.get(3, 0)
rev_rem = queue_counts.get(2, 0)
con_anki.close()

WHOLE_MINUTES = 236  # 236분 43.47초 (14,203.47초 / 3시간 56분 43초, 236.72분)

SUMMARY_PAYLOAD = {
    'schema': 'daily_summary_v1',
    'scope': 'all_decks',
    'scopeLabel': '전체 덱',
    'answeredCards': 2777,
    'studyMinutes': 236.72,
    'secondsPerCard': 5.11,
    'againCount': 2332,
    'againPct': 83.98,
    'learningReviews': 2369,  # 938 learn + 1431 relearn
    'reviewsDone': 408,
    'ratings': {
        'again': 2332,
        'hard': 20,
        'good': 34,
        'easy': 391
    },
    'cardsRemaining': {
        'new': new_rem,
        'learning': learn_rem,
        'review': rev_rem
    },
    'studyDay': '2026-09-16',
    'ankiResourceStudyDayRaw': '2026-09-16',
    'sourceNote': 'Anki revlog 2026-09-16 실측치 (총 2,777회, 236분 43.47초 / 3시간 56분 43초 / 236.72분). 옵션 A 11일차: N2 신규 66장 100% 완수(262->196, 누적 83.25%), N3 신규 19장 100% 완수(19->0, 누적 100% N3 전량 1차 완독 달성!). N4 18장, N5 18장 선행 점검. 당일 신규 노출 총 85장. 잔여 학습 대기(Learning queue=1) 76장(N2 60, N3 16)은 내일 큐로 정상 이월.',
    'inventory': {
        'n3_unseen': 0,
        'n2_unseen': 196,
        'n2_new_today': 66,
        'n3_new_today': 19,
        'grammar_new_today': 0,
        'grammar_reviewed_today': 0,
        'grammar_unseen': 0,
        'new_cards_exposed_today': 85
    }
}

SESSION_SUMMARY = (
    '[2026-09-16] Anki 단어 세션 2,777회(236분 43초, 3시간 56분 43초 / 236.72분). '
    '★N3 전량 1차 완독 달성(미학습 19->0장, 누적 100%)★ + '
    'N2 신규 66장 100% 완수(262->196장, 누적 83.25%). '
    '당일 신규 노출 총 85장 100% 진입. 잔여 재학습 큐 76장(N2 60, N3 16)은 다음날 복습 큐로 정상 이월. '
    '잔여 43.47초 보존.'
)

con = sqlite3.connect(DB)
with con:
    # 1. Update or Insert study_sessions
    row = con.execute('SELECT verified_minutes, summary FROM study_sessions WHERE session_date=?', (DATE,)).fetchone()
    if row is None:
        con.execute(
            'INSERT INTO study_sessions (session_date, verified_minutes, has_untracked_activity, summary, updated_at) '
            'VALUES (?, ?, 0, ?, CURRENT_TIMESTAMP)',
            (DATE, WHOLE_MINUTES, SESSION_SUMMARY)
        )
    else:
        con.execute(
            'UPDATE study_sessions SET verified_minutes=?, summary=?, updated_at=CURRENT_TIMESTAMP WHERE session_date=?',
            (WHOLE_MINUTES, SESSION_SUMMARY, DATE)
        )

    # 2. Update or Insert anki_daily_stats
    anki_row = con.execute('SELECT snapshot_date FROM anki_daily_stats WHERE snapshot_date=?', (DATE,)).fetchone()
    if anki_row:
        con.execute(
            'UPDATE anki_daily_stats SET scope_label=?, payload_json=?, source_filename=?, updated_at=CURRENT_TIMESTAMP WHERE snapshot_date=?',
            ('전체 덱', json.dumps(SUMMARY_PAYLOAD, ensure_ascii=False), 'Anki revlog snapshot 2026-09-16', DATE)
        )
    else:
        con.execute(
            'INSERT INTO anki_daily_stats (snapshot_date, scope_label, payload_json, source_filename, captured_at, updated_at) '
            'VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)',
            (DATE, '전체 덱', json.dumps(SUMMARY_PAYLOAD, ensure_ascii=False), 'Anki revlog snapshot 2026-09-16')
        )

    assert con.execute('PRAGMA integrity_check').fetchone()[0] == 'ok'
    assert not con.execute('PRAGMA foreign_key_check').fetchall()

print('Successfully updated 2026-09-16 stats (236.72 min, 2777 reviews) to DB!')
cur = con.cursor()
cur.execute('SELECT * FROM study_sessions WHERE session_date=?', (DATE,))
print('study_sessions:', cur.fetchall())
con.close()
