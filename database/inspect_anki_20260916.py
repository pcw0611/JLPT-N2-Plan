# -*- coding: utf-8 -*-
"""Inspect Anki reviews and calculate statistics for 2026-09-16 study day."""
import sqlite3
import shutil
import json
import os
...
start_dt = datetime(2026, 9, 16, 4, 0, 0, tzinfo=KST)
end_dt = datetime(2026, 9, 17, 4, 0, 0, tzinfo=KST)
...
rows = cur.fetchall()
print(f'Total reviews for 2026-09-16 study day: {len(rows)}')
...
print('\nCurrent queue counts:', dict(cur.fetchall()))
...
# Check unseen cards in N2 and N3
print('\n--- Unseen/Remaining in relevant decks ---')
for did, dname in deck_names.items():
    if any(k in dname for k in ['N2', 'N3', '4-N2', '3-N3', '일문따']):
